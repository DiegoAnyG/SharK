...
"""
    ...
    if run_now:
        ...
        if run_script.is_file():
            ...
            # Expected total steps per simulation phase (dt = 0.002 ps)
            phase_specs = [
                ("EM", ["em", "energy min"], 5000),
                ("NVT", ["nvt"], 50000),
                ("NPT", ["npt"], 50000),
                ("Production MD", ["production", "prod"], int(round(sim_time_ns * 1000 / 0.002))),
            ]
            ...
            with open(log_file, "w", encoding="utf-8") as log_f:
                ...
                step_pattern = re.compile(r"(?:Step\s*=?\s*|step\s+)(\d+)", re.IGNORECASE)
                rem_pattern = re.compile(r"remaining\s+(?:runtime|time):\s*([^\r\n,]+)", re.IGNORECASE)
                finish_pattern = re.compile(r"will\s+finish\s+([^\r\n,]+)", re.IGNORECASE)

                if proc.stdout is not None:
                    for raw_line in iter(proc.stdout.readline, ""):
                        log_f.write(raw_line)
                        log_f.flush()
                        all_logs.append(raw_line)
                        line = raw_line.strip()
                        if not line:
                            continue

                        # Phase and step announcements
                        if line.startswith("### ") and line.endswith(" ###"):
                            phase_label = line.strip("#").strip()
            ...
            if proc.returncode == 0:
                ...
                status = "failed"
                print(f"[SharK-MD] Simulation {run_id} failed (exit code {proc.returncode}). Log saved to {log_file}")
                err_lines = [l.strip() for l in all_logs[-15:] if l.strip()]
                if err_lines:
